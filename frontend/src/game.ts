...
export class Game {
  ...
  constructor(
    ...
    this._currentPoints = guessedWords
      .map(Game.calculateWordScore)
      .reduce(sum, 0);
  }
  ...
  public static getLatestGame(): string | null {
    return localStorage.getItem("game/latest");
  }

  public setLatestGame(): void {
    localStorage.setItem("game/latest", this._letters);
  }
  ...
  public static load(letters: string): Game {
    const totalPoints = parseInt(
      localStorage.getItem(`game/${letters}/totalPoints`)!,
      10
    );
    const guessedWords = localStorage
      .getItem(`game/${letters}/guessedWords`)
      ?.split(Game.SEPARATOR)
      .filter((x) => !!x && x !== "");
  ...
  public save(): void {
    localStorage.setItem(
      `game/${this._letters}/totalPoints`,
      this._totalPoints.toString()
    );
    localStorage.setItem(
      `game/${this._letters}/guessedWords`,
      this._guessedWords.join(Game.SEPARATOR)
    );
    this.setLatestGame();
    ...
    if (existingIds == null) {
      ...
      localStorage.setItem(
        "game/ids",
        [...existingIds, this._letters].join(Game.SEPARATOR)
      );
    }
  }
