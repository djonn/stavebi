...
if (args.Count() == 1 && args[0].ToLower() == "lemma-count")
{
  var generator = new GameGenerator();

  var allWords = generator.ParseWords().Where(generator.isValidWord);
  var allCount = allWords.Count();
  var fullFormCount = allWords.Select(x => x.FullForm).Distinct().Count();
  var lemmaCount = allWords.Select(x => x.Lemma).Distinct().Count();

  Console.WriteLine("All count: {0}", allCount);
